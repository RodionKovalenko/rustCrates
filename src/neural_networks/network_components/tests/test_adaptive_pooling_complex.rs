...
mod test_adaptive_pooling_complex {
    ...
    use crate::neural_networks::{
        network_components::{
            adaptive_pooling_complex::{calculate_reconstruction_error, create_random_input, test_decompression_methods, AdaptiveAvgPool1d, ContentAwarePooling, DynamicSequenceCompressor, InterpolationDecompressor, StridedPooling},
    ...
    fn test_adaptive_avg_pooling_complex() {
        ...
        let pool: AdaptiveAvgPool1d = AdaptiveAvgPool1d::new(45);
    ...
    fn test_benchmarks() {
        ...
        let adaptive_pool = AdaptiveAvgPool1d::new(45);

        // Compress
        let (compressed, metadata) = adaptive_pool.compress(&input);
        ...
        // Decompress
        let decompressed = adaptive_pool.decompress(&compressed, &metadata);
        ...
        let dynamic_compressor = DynamicSequenceCompressor::new(40, 50);

        let (compressed2, metadata2) = dynamic_compressor.compress(&input);
        println!("Dynamic compressed shape: [{}, {}, {}]", compressed2.len(), compressed2[0].len(), compressed2[0][0].len());

        let decompressed2 = dynamic_compressor.decompress(&compressed2, &metadata2);
        ...
        println!("\n=== Test 3: Strided Pooling Compression/Decompression ===");
        let strided_pool = StridedPooling::new(21, Some(21));
        ...
        println!("\n=== Test 4: Content-Aware Compression/Decompression ===");
        let content_aware = ContentAwarePooling::new(45);
        ...
        println!("\n=== Test 5: Linear Interpolation Decompression ===");
        let interpolated = InterpolationDecompressor::linear_interpolate(&compressed, seq_len);
        ...
        for &length in &test_lengths {
            ...
            let (compressed_test, metadata_test) = adaptive_pool.compress(&test_input);
            let decompressed_test = adaptive_pool.decompress(&compressed_test, &metadata_test);
    ...
    fn test_adaptive_avg_pooling_gradient() {
        ...
        // Adaptive pooling to exactly 45 tokens
        let pool: AdaptiveAvgPool1d = AdaptiveAvgPool1d::new(16);
        let mut softmax_layer: SoftmaxLayer = SoftmaxLayer::new(learning_rate, operation_mode);
        let (compressed, metadata) = pool.forward(&input_batch);
        ...
        let anal_linear_gradient_input_batch = pool.backward(&gradient_softmax.get_gradient_input_batch(), &metadata);

        // TEST GRADIENT OF THE INPUT BATCH
        // Define the loss function
        let mut loss_fn = |input: &Vec<Vec<Vec<Complex<f64>>>>| -> Complex<f64> {
            let (compressed, _metadata) = pool.forward(&input);
            let _softmax_batch_output: Vec<Vec<Vec<f64>>> = softmax_layer.forward(&compressed, Some(padding_mask_batch.clone()));
