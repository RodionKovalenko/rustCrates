    ...
    for num_layer in 0..total_number_of_layers {
        ...
        let input_layer: Layer<T> = Layer {
            input_weights: initialize_weights(num_columns,
                                              num_hidden_neurons),
            inactivated_output: create_generic_3d(num_rows, num_hidden_neurons, feed_net.minibatch_size),
            activated_output: create_generic_3d(num_rows, num_hidden_neurons, feed_net.minibatch_size),
            layer_bias: create_generic_one_dim(num_hidden_neurons),
            gradient: create_generic(num_columns, num_hidden_neurons),
            errors: create_generic(feed_net.minibatch_size, num_hidden_neurons),
            activation_type: ActivationType::TANH,
            layer_type,
            previous_layer: None,
            next_layer: None,
            input_data: create_generic_3d(num_rows, num_hidden_neurons, feed_net.minibatch_size),
