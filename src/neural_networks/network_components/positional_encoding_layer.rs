    ...
    pub fn forward_rm(&mut self, layer_input: &LayerInput) -> Vec<RowMajorMatrix<Complex<f64>>> {
        ...
        if layer_input.get_calculate_gradient() {
            self.input_batch_rm = Some(input_batch_rm.to_vec());
                pub fn forward_rm(&mut self, layer_input: &LayerInput) -> Vec<RowMajorMatrix<C>> {
                    let input_batch_rm = layer_input
                        .get_input_batch_rm_ref()
                        .expect("PositionalEncodingLayer::forward_rm: input_batch_rm missing");
            ...
            .map(|m| {
                ...
                for position in 0..m.rows {
                    ...
                    for i in 0..half_dim {
                        ...
                        let mut theta = time_step as f64
                        ...
                        out.data[out_row.start + even_idx] =
                            C::new(even.re * cos_theta - odd.re * sin_theta, even.im * cos_theta - odd.im * sin_theta);
                        out.data[out_row.start + odd_idx] =
                            C::new(even.re * sin_theta + odd.re * cos_theta, even.im * sin_theta + odd.im * cos_theta);
                    }
                }
    ...
    pub fn backward(&mut self, previous_gradient_batch: &Vec<Vec<Vec<C>>>) -> Gradient {
        if self.input_batch.is_none() {
            if self.input_batch_rm.is_some() {
                ...
                let mut gradient = self.backward_rm(&previous_gradient_batch_rm);
                let legacy_gx: Vec<Vec<Vec<C>>> = gradient
                    .get_gradient_input_batch_rm()
                    .iter()
                    .map(|m| m.to_rows())
                    .collect();
                gradient.set_gradient_input_batch(legacy_gx);
