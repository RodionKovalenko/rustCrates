...
impl MultiLinearLayer {
    ...
    pub fn forward(&mut self, input: &LayerInput) -> LayerOutput {
        ...
        let (batch_size, seq_len, input_batch_rm, legacy_mode): (usize, usize, Vec<RowMajorMatrix<Complex<f64>>>, bool) =
            if let Some(rm) = input_batch_rm_ref {
                ...
                let input_batch_rm: Vec<RowMajorMatrix<Complex<f64>>> = input_batch.iter().map(|sample| RowMajorMatrix::from_rows(sample)).collect();
        ...
        let output_chunks_rm: Vec<Vec<RowMajorMatrix<Complex<f64>>>> = self
            .layers
            .iter_mut()
            .map(|lin_layer| {
        ...
        let mut output_batch_rm: Vec<RowMajorMatrix<Complex<f64>>> = Vec::with_capacity(batch_size);
        for b in 0..batch_size {
            let mut data = vec![Complex::new(0.0, 0.0); seq_len * output_feature_size];
            for r in 0..seq_len {
                let mut offset = 0;
    ...
    pub fn backward(&mut self, previous_gradient: &Gradient) -> Gradient {
        ...
        if let Some(prev_grad_rm) = prev_grad_rm_ref {
            let mut summed_dx: Vec<RowMajorMatrix<Complex<f64>>> = (0..batch_size)
                .map(|_| RowMajorMatrix::from_data(seq_len, feature_dim, vec![Complex::new(0.0, 0.0); seq_len * feature_dim]))
                .collect();
            ...
            for (layer_ind, lin_layer) in self.layers.iter_mut().enumerate() {
                ...
                let sliced_prev: Vec<RowMajorMatrix<Complex<f64>>> = prev_grad_rm
                    .iter()
                    .map(|m| {
                        let mut data = vec![Complex::new(0.0, 0.0); seq_len * chunk_cols];
                        for r in 0..seq_len {
        ...
        for (layer_ind, lin_layer) in self.layers.iter_mut().enumerate() {
            let (start_col, end_col) = self.col_ranges[layer_ind];
            let prev_chunk: Vec<Vec<Vec<Complex<f64>>>> = previous_input_gradient
                .iter()
        ...
        let gradient_input_batch: Vec<Vec<Vec<Complex<f64>>>> = summed_dx.iter().map(|m| m.to_rows()).collect();
