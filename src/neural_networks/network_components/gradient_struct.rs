...
pub enum GradientBatch {
    Complex(Vec<Vec<Vec<C>>>),
    Real(Vec<Vec<Vec<Real>>>),
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Gradient {
    gradient_weights_batch: Option<Vec<Vec<Vec<C>>>>,
    gradient_weights_2_batch: Option<Vec<Vec<Vec<C>>>>,
    gradient_input_batch: Option<Vec<Vec<Vec<C>>>>,
    #[serde(skip)]
    gradient_input_batch_rm: Option<Vec<RowMajorMatrix<C>>>,
    gradient_input_batch_softmax: Option<Vec<Vec<Vec<Real>>>>,
    gradient_bias_batch: Option<Vec<Vec<C>>>,
    gradient_gamma_batch: Option<Vec<Vec<C>>>,
    gradient_beta_batch: Option<Vec<Vec<C>>>,

    gradient_weights: Option<Vec<Vec<C>>>,
    gradient_weights_vec_batch_1: Option<Vec<Vec<C>>>,
    gradient_weights_vec_batch_2: Option<Vec<Vec<C>>>,

    gradient_weights_vec_1: Option<Vec<C>>,
    gradient_weights_vec_2: Option<Vec<C>>,

    gradient_weights_2: Option<Vec<Vec<C>>>,
    gradient_input: Option<Vec<Vec<C>>>,
    gradient_bias: Option<Vec<C>>,
    gradient_gamma: Option<Vec<C>>,
    gradient_beta: Option<Vec<C>>,

    gradient_weights_q_batch: Option<Vec<Vec<Vec<C>>>>,
    gradient_weights_v_batch: Option<Vec<Vec<Vec<C>>>>,
    gradient_weights_k_batch: Option<Vec<Vec<Vec<C>>>>,

    gradient_bias_pos_batch: Option<Vec<Vec<Vec<C>>>>,

    gradient_weights_q: Option<Vec<Vec<C>>>,
    gradient_weights_v: Option<Vec<Vec<C>>>,
    gradient_weights_k: Option<Vec<Vec<C>>>,

    gradient_bias_pos: Option<Vec<Vec<C>>>,

    prev_m_weights_q: Option<Vec<Vec<C>>>,
    prev_m_weights_k: Option<Vec<Vec<C>>>,
    prev_m_weights_v: Option<Vec<Vec<C>>>,
    prev_m_bias_pos: Option<Vec<Vec<C>>>,

    prev_v_weights_q: Option<Vec<Vec<C>>>,
    prev_v_weights_k: Option<Vec<Vec<C>>>,
    prev_v_weights_v: Option<Vec<Vec<C>>>,
    prev_v_bias_pos: Option<Vec<Vec<C>>>,

    prev_m_weights: Option<Vec<Vec<C>>>,
    prev_v_weights: Option<Vec<Vec<C>>>,

    prev_m_weights_2: Option<Vec<Vec<C>>>,
    prev_v_weights_2: Option<Vec<Vec<C>>>,

    prev_m_weights_vec_1: Option<Vec<C>>,
    prev_v_weights_vec_1: Option<Vec<C>>,

    prev_v_weights_vec_hat_1: Option<Vec<C>>,
    prev_v_weights_vec_hat_2: Option<Vec<C>>,

    prev_m_weights_vec_2: Option<Vec<C>>,
    prev_v_weights_vec_2: Option<Vec<C>>,

    prev_v_weights_hat: Option<Vec<Vec<C>>>,
    prev_v_weights_hat_2: Option<Vec<Vec<C>>>,

    prev_v_weights_q_hat: Option<Vec<Vec<C>>>,
    prev_v_weights_k_hat: Option<Vec<Vec<C>>>,
    prev_v_weights_v_hat: Option<Vec<Vec<C>>>,
    prev_v_weights_p_hat: Option<Vec<Vec<C>>>,

    prev_v_gamma_hat: Option<Vec<C>>,
    prev_v_beta_hat: Option<Vec<C>>,

    prev_m_bias: Option<Vec<C>>,
    prev_v_bias: Option<Vec<C>>,
    prev_v_bias_hat: Option<Vec<C>>,

    prev_m_gamma: Option<Vec<C>>,
    prev_v_gamma: Option<Vec<C>>,

    prev_m_beta: Option<Vec<C>>,
    prev_v_beta: Option<Vec<C>>,
...
impl Gradient {
    ...
    pub fn set_gradient_input_batch(&mut self, gradient_input_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_input_batch = Some(gradient_input_batch);
    }

    pub fn set_gradient_input_batch_rm(&mut self, gradient_input_batch_rm: Vec<RowMajorMatrix<C>>) {
        self.gradient_input_batch_rm = Some(gradient_input_batch_rm);
    }
    pub fn set_gradient_input_batch_softmax(&mut self, gradient_input_batch: Vec<Vec<Vec<Real>>>) {
        self.gradient_input_batch_softmax = Some(gradient_input_batch);
    }
    pub fn set_gradient_weight_batch(&mut self, gradient_weight_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_weights_batch = Some(gradient_weight_batch);
    }
    pub fn set_gradient_weight_2_batch(&mut self, gradient_weight_2_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_weights_2_batch = Some(gradient_weight_2_batch);
    }
    pub fn set_gradient_bias_batch(&mut self, gradient_bias_batch: Vec<Vec<C>>) {
        self.gradient_bias_batch = Some(gradient_bias_batch);
    }
    pub fn set_gradient_gamma_batch(&mut self, gradient_gamma_batch: Vec<Vec<C>>) {
        self.gradient_gamma_batch = Some(gradient_gamma_batch);
    }
    pub fn set_gradient_beta_batch(&mut self, gradient_beta_batch: Vec<Vec<C>>) {
        self.gradient_beta_batch = Some(gradient_beta_batch);
    }

    pub fn set_gradient_input(&mut self, gradient_input: Vec<Vec<C>>) {
        self.gradient_input = Some(gradient_input);
    }
    pub fn set_gradient_weights(&mut self, gradient_weights: Vec<Vec<C>>) {
        self.gradient_weights = Some(gradient_weights);
    }
    pub fn set_gradient_weights_2(&mut self, gradient_weights_2: Vec<Vec<C>>) {
        self.gradient_weights_2 = Some(gradient_weights_2);
    }
    pub fn set_gradient_bias(&mut self, gradient_bias: Vec<C>) {
        self.gradient_bias = Some(gradient_bias);
    }
    pub fn set_gradient_gamma(&mut self, gradient_gamma: Vec<C>) {
        self.gradient_gamma = Some(gradient_gamma);
    }
    pub fn set_gradient_beta(&mut self, gradient_beta: Vec<C>) {
    ...
    pub fn set_gradient_weights_q_batch(&mut self, gradient_weights_q_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_weights_q_batch = Some(gradient_weights_q_batch);
    }
    pub fn set_gradient_weights_v_batch(&mut self, gradient_weights_v_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_weights_v_batch = Some(gradient_weights_v_batch);
    }
    pub fn set_gradient_bias_pos_batch(&mut self, bias_pos_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_bias_pos_batch = Some(bias_pos_batch);
    }

    pub fn set_gradient_bias_pos(&mut self, bias_pos: Vec<Vec<C>>) {
        self.gradient_bias_pos = Some(bias_pos);
    }
    pub fn set_gradient_weights_k_batch(&mut self, gradient_weights_k_batch: Vec<Vec<Vec<C>>>) {
        self.gradient_weights_k_batch = Some(gradient_weights_k_batch);
    }

    pub fn set_gradient_q(&mut self, gradient_weights_q: Vec<Vec<C>>) {
        self.gradient_weights_q = Some(gradient_weights_q);
    }
    pub fn set_gradient_v(&mut self, gradient_weights_v: Vec<Vec<C>>) {
        self.gradient_weights_v = Some(gradient_weights_v);
    }
    pub fn set_gradient_k(&mut self, gradient_weights_k: Vec<Vec<C>>) {
        self.gradient_weights_k = Some(gradient_weights_k);
    }
    pub fn set_prev_m_weights(&mut self, prev_m_weights: Vec<Vec<C>>) {
        self.prev_m_weights = Some(prev_m_weights);
    }
    pub fn set_prev_v_weights(&mut self, prev_v_weights: Vec<Vec<C>>) {
        self.prev_v_weights = Some(prev_v_weights);
    }

    pub fn set_prev_m_weights_2(&mut self, prev_m_weights_2: Vec<Vec<C>>) {
        self.prev_m_weights_2 = Some(prev_m_weights_2);
    }
    pub fn set_prev_v_weights_2(&mut self, prev_v_weights_2: Vec<Vec<C>>) {
        self.prev_v_weights_2 = Some(prev_v_weights_2);
    }
    pub fn set_gradient_weights_vec_batch_1(&mut self, gradient_weights_vec_1: Vec<Vec<C>>) {
        self.gradient_weights_vec_batch_1 = Some(gradient_weights_vec_1);
    }
    pub fn set_gradient_weights_vec_batch_2(&mut self, gradient_weights_vec_2: Vec<Vec<C>>) {
        self.gradient_weights_vec_batch_2 = Some(gradient_weights_vec_2);
    }
    pub fn set_gradient_weights_vec_1(&mut self, gradient_weights_vec_1: Vec<C>) {
        self.gradient_weights_vec_1 = Some(gradient_weights_vec_1);
    }
    pub fn set_gradient_weights_vec_2(&mut self, gradient_weights_vec_2: Vec<C>) {
        self.gradient_weights_vec_2 = Some(gradient_weights_vec_2);
    }

    pub fn set_prev_m_weights_vec_1(&mut self, prev_m_weights_vec_1: Vec<C>) {
        self.prev_m_weights_vec_1 = Some(prev_m_weights_vec_1);
    }
    pub fn set_prev_v_weights_vec_1(&mut self, prev_v_weights_vec_1: Vec<C>) {
        self.prev_v_weights_vec_1 = Some(prev_v_weights_vec_1);
    }

    pub fn set_prev_m_weights_vec_2(&mut self, prev_m_weights_vec_2: Vec<C>) {
        self.prev_m_weights_vec_2 = Some(prev_m_weights_vec_2);
    }
    pub fn set_prev_v_weights_vec_2(&mut self, prev_v_weights_vec_2: Vec<C>) {
        self.prev_v_weights_vec_2 = Some(prev_v_weights_vec_2);
    }

    pub fn set_prev_v_weights_vec_hat_1(&mut self, prev_v_weights_vec_hat_1: Vec<C>) {
        self.prev_v_weights_vec_hat_1 = Some(prev_v_weights_vec_hat_1);
    }
    pub fn set_prev_v_weights_vec_hat_2(&mut self, prev_v_weights_vec_hat_2: Vec<C>) {
        self.prev_v_weights_vec_hat_2 = Some(prev_v_weights_vec_hat_2);
    }

    pub fn set_prev_m_weights_q(&mut self, prev_m_weights_q: Vec<Vec<C>>) {
        self.prev_m_weights_q = Some(prev_m_weights_q);
    }
    pub fn set_prev_m_weights_k(&mut self, prev_m_weights_k: Vec<Vec<C>>) {
        self.prev_m_weights_k = Some(prev_m_weights_k);
    }
    pub fn set_prev_m_weights_v(&mut self, prev_m_weights_v: Vec<Vec<C>>) {
        self.prev_m_weights_v = Some(prev_m_weights_v);
    }
    pub fn set_prev_m_bias_pos(&mut self, prev_m_bias_pos: Vec<Vec<C>>) {
        self.prev_m_bias_pos = Some(prev_m_bias_pos);
    }

    pub fn set_prev_v_weights_q(&mut self, prev_v_weights_q: Vec<Vec<C>>) {
        self.prev_v_weights_q = Some(prev_v_weights_q);
    }
    pub fn set_prev_v_weights_k(&mut self, prev_v_weights_k: Vec<Vec<C>>) {
        self.prev_v_weights_k = Some(prev_v_weights_k);
    }
    pub fn set_prev_v_weights_v(&mut self, prev_v_weights_v: Vec<Vec<C>>) {
        self.prev_v_weights_v = Some(prev_v_weights_v);
    }
    pub fn set_prev_v_bias_pos(&mut self, prev_v_bias_pos: Vec<Vec<C>>) {
        self.prev_v_bias_pos = Some(prev_v_bias_pos);
    }

    pub fn set_prev_m_bias(&mut self, prev_m_bias: Vec<C>) {
        self.prev_m_bias = Some(prev_m_bias);
    }
    pub fn set_prev_v_bias(&mut self, prev_v_bias: Vec<C>) {
        self.prev_v_bias = Some(prev_v_bias);
    }

    pub fn set_prev_m_beta(&mut self, prev_m_beta: Vec<C>) {
        self.prev_m_beta = Some(prev_m_beta);
    }
    pub fn set_prev_v_beta(&mut self, prev_v_beta: Vec<C>) {
        self.prev_v_beta = Some(prev_v_beta);
    }

    pub fn set_prev_v_bias_hat(&mut self, prev_v_bias_hat: Vec<C>) {
        self.prev_v_bias_hat = Some(prev_v_bias_hat);
    }

    pub fn set_prev_v_beta_hat(&mut self, prev_v_beta_hat: Vec<C>) {
        self.prev_v_beta_hat = Some(prev_v_beta_hat);
    }
    pub fn set_prev_v_gamma_hat(&mut self, prev_v_gamma_hat: Vec<C>) {
        self.prev_v_gamma_hat = Some(prev_v_gamma_hat);
    }

    pub fn set_prev_v_weights_hat(&mut self, prev_v_weights_hat: Vec<Vec<C>>) {
        self.prev_v_weights_hat = Some(prev_v_weights_hat);
    }

    pub fn set_prev_v_weights_hat_2(&mut self, prev_v_weights_hat_2: Vec<Vec<C>>) {
        self.prev_v_weights_hat_2 = Some(prev_v_weights_hat_2);
    }

    pub fn set_prev_v_weights_q_hat(&mut self, prev_v_weights_q_hat: Vec<Vec<C>>) {
        self.prev_v_weights_q_hat = Some(prev_v_weights_q_hat);
    }
    pub fn set_prev_v_weights_k_hat(&mut self, prev_v_weights_k_hat: Vec<Vec<C>>) {
        self.prev_v_weights_k_hat = Some(prev_v_weights_k_hat);
    }
    pub fn set_prev_v_weights_v_hat(&mut self, prev_v_weights_v_hat: Vec<Vec<C>>) {
        self.prev_v_weights_v_hat = Some(prev_v_weights_v_hat);
    }
    pub fn set_prev_v_weights_p_hat(&mut self, prev_v_weights_p_hat: Vec<Vec<C>>) {
        self.prev_v_weights_p_hat = Some(prev_v_weights_p_hat);
    }

    pub fn set_prev_m_gamma(&mut self, prev_m_gamma: Vec<C>) {
        self.prev_m_gamma = Some(prev_m_gamma);
    }
    pub fn set_prev_v_gamma(&mut self, prev_v_gamma: Vec<C>) {
    ...
    pub fn get_gradient_input_batch(&self) -> Vec<Vec<Vec<C>>> {
    ...
    pub fn get_gradient_input_batch_rm(&self) -> Vec<RowMajorMatrix<C>> {
        if let Some(gradient_input_batch_rm) = &self.gradient_input_batch_rm {
    ...
    pub fn get_gradient_input_batch_rm_ref(&self) -> Option<&[RowMajorMatrix<C>]> {
        self.gradient_input_batch_rm.as_deref()
    }

    pub fn get_gradient_input_batch_ref(&self) -> Option<&[Vec<Vec<C>>]> {
        self.gradient_input_batch.as_deref()
    }
    pub fn get_gradient_input_batch_softmax(&self) -> Vec<Vec<Vec<Real>>> {
        self.gradient_input_batch_softmax.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_input_softmax(&self) -> Vec<Vec<Real>> {
        if let Some(gradient_input_batch_softmax) = self.gradient_input_batch_softmax.clone() {
            self.group_gradient_batch_real(&gradient_input_batch_softmax)
        } else {
            vec![]
        }
    }
    pub fn get_gradient_weight_batch(&self) -> Vec<Vec<Vec<C>>> {
        self.gradient_weights_batch.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_weight_2_batch(&self) -> Vec<Vec<Vec<C>>> {
        self.gradient_weights_2_batch.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_bias_batch(&self) -> Vec<Vec<C>> {
        self.gradient_bias_batch.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_gamma_batch(&self) -> Vec<Vec<C>> {
        self.gradient_gamma_batch.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_beta_batch(&self) -> Vec<Vec<C>> {
        self.gradient_beta_batch.clone().unwrap_or_else(|| vec![])
    }

    pub fn get_gradient_weights_q_batch(&self) -> Vec<Vec<Vec<C>>> {
        self.gradient_weights_q_batch.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_weights_v_batch(&self) -> Vec<Vec<Vec<C>>> {
        self.gradient_weights_v_batch.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_gradient_weights_k_batch(&self) -> Vec<Vec<Vec<C>>> {
        self.gradient_weights_k_batch.clone().unwrap_or_else(|| vec![])
    }

    pub fn get_gradient_bias_pos_batch(&self) -> Vec<Vec<Vec<C>>> {
        self.gradient_bias_pos_batch.clone().unwrap_or_else(|| vec![])
    }

    pub fn get_prev_m_weights(&self) -> Vec<Vec<C>> {
        self.prev_m_weights.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_prev_v_weights(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_prev_m_beta(&self) -> Vec<C> {
        self.prev_m_beta.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_prev_v_beta(&self) -> Vec<C> {
        self.prev_v_beta.clone().unwrap_or_else(|| vec![])
    }

    pub fn get_prev_m_gamma(&self) -> Vec<C> {
        self.prev_m_gamma.clone().unwrap_or_else(|| vec![])
    }
    pub fn get_prev_v_gamma(&self) -> Vec<C> {
        self.prev_v_gamma.clone().unwrap_or_else(|| vec![])
    }

    pub fn get_gradient_weights_q(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_weights_v(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_weights_k(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_bias_pos(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_weights(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_weights_2(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_input(&self) -> Vec<Vec<C>> {
    ...
    pub fn get_gradient_bias(&self) -> Vec<C> {
    ...
    pub fn get_gradient_weights_vec_1(&self) -> Vec<C> {
    ...
    pub fn get_gradient_weights_vec_2(&self) -> Vec<C> {
    ...
    pub fn get_gradient_gamma(&self) -> Vec<C> {
    ...
    pub fn get_gradient_beta(&self) -> Vec<C> {
    ...
    pub fn group_gradient_batch_real(&self, weight_gradients_batch: &Vec<Vec<Vec<Real>>>) -> Vec<Vec<Real>> {
        if weight_gradients_batch.is_empty() || weight_gradients_batch[0].is_empty() || weight_gradients_batch[0][0].is_empty() {
            return vec![];
        }

        let mut weight_gradients: Vec<Vec<Real>> = vec![vec![ZERO; weight_gradients_batch[0][0].len()]; weight_gradients_batch[0].len()];
    ...
    pub fn group_gradient_batch_bias(&self, bias_gradient_batch: &Vec<Vec<C>>) -> Vec<C> {
        if bias_gradient_batch.is_empty() || bias_gradient_batch[0].is_empty() {
            return vec![];
        }

        let mut bias_gradients: Vec<C> = vec![C::new(ZERO, ZERO); bias_gradient_batch[0].len()];
    ...
    pub fn group_array_batch(weight_gradients_batch: &Vec<Vec<Vec<C>>>) -> Vec<Vec<C>> {
        let mut weight_gradients: Vec<Vec<C>> = vec![vec![C::new(ZERO, ZERO); weight_gradients_batch[0][0].len()]; weight_gradients_batch[0].len()];
