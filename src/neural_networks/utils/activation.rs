...
pub fn sigmoid_generic<T: Debug + Clone + From<f64> + Into<f64> +
Mul<Output=T> + AddAssign + Add<Output=T> + Div<Output=T>>(matrix_a: &Vec<Vec<T>>) -> Vec<Vec<T>> {
    ...
    for j in 0..matrix_a[0].len() {
        for i in 0..matrix_a.len() {
            result_matrix[i][j] = sigmoid_value_generic(matrix_a[i][j].clone());
        }
    }
...
pub fn sigmoid(matrix_a: &Vec<Vec<f64>>) -> Vec<Vec<f64>> {
    let mut result_matrix = matrix_a.clone();

    for j in 0..matrix_a[0].len() {
        for i in 0..matrix_a.len() {
            result_matrix[i][j] = sigmoid_value(matrix_a[i][j]);
        }
    }
...
pub fn sigmoid_value_generic<T: Debug + Clone + From<f64> + Into<f64> +
...
pub fn sigmoid_value(value: f64) -> f64 {
    1.0 / 1.0 + E.powf(-1.0 * value as f32) as f64
}
