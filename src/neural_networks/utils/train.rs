...
pub fn calculate_gradient(layers: &mut Vec<Layer<f64>>,
    ...
    if matches!(layers[layer_ind].layer_type, LayerType::OutputLayer) {
        // calculate gradients and error
        for inp_set_ind in minibatch_start..minibatch_end {
            for j in 0..layers[layer_ind].input_weights[0].len() {
        ...
        for j in 0..layers[layer_ind].input_weights[0].len() {
            for i in 0..layers[layer_ind].input_weights.len() {
                ...
                m1 = get_adam_value(&layers[layer_ind].gradient[i][j], b1,
                                    layers[layer_ind].m1[i][j]);
                v1 = get_r_rms_value(
                    &layers[layer_ind].gradient[i][j],
                    b2, layers[layer_ind].v1[i][j]);

                m_hat = m1 / (1.0 - b1.powf((iter + 1) as f64));
                v_hat = v1 / (1.0 - b2.powf((iter + 1) as f64));

                delta_theta = ((learn_rate * m_hat) / (v_hat.sqrt() + e));
                layers[layer_ind].input_weights[i][j] -= delta_theta;

                layers[layer_ind].m1[i][j] = m1;
                layers[layer_ind].v1[i][j] = v1;
            }
            // update bias
            for inp_set_ind in minibatch_start..minibatch_end {
                layers[layer_ind].layer_bias[j] -=
                    (learn_rate * layers[layer_ind].errors[inp_set_ind][j])
                        / num_sets as f64;
            }
        }
    } else {
        for inp_set_ind in minibatch_start..minibatch_end {
            for j in 0..layers[layer_ind].input_weights[0].len() {
                ...
                for d in 0..layers[layer_ind].activated_output[inp_set_ind].len() {
                    ...
                    layers[layer_ind].errors[inp_set_ind][j] *= get_derivative(
                        layers[layer_ind].activated_output[inp_set_ind][d][j],
                        layers[layer_ind].activation_type.clone());
                }
                ...
                for i in 0..layers[layer_ind].input_weights.len() {
                    ...
                    for d in 0..layers[layer_ind].activated_output[inp_set_ind].len() {
                        layers[layer_ind].gradient[i][j] +=
                            (layers[layer_ind].errors[inp_set_ind][j] *
                                layers[layer_ind].input_data[inp_set_ind][d][i])
                                / num_sets as f64;
                    }
                }
            }
        ...
        for j in 0..layers[layer_ind].input_weights[0].len() {
            for i in 0..layers[layer_ind].input_weights.len() {
                ...
                m1 = get_adam_value(
                    &layers[layer_ind].gradient[i][j], b1,
                    layers[layer_ind].m1[i][j]);
                v1 = get_r_rms_value(
                    &layers[layer_ind].gradient[i][j], b2,
                    layers[layer_ind].v1[i][j]);
