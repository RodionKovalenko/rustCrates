...
n_steps = 15
look_back = n_steps
look_back_focast = 300
current_abs_path = str(pathlib.Path().resolve())

datatime = datetime.datetime.today()
datestr = str(datatime.day) + '.' + str(datatime.month) + '.' + str(datatime.year) + '-' + str(datatime.hour) + '-' + str(datatime.minute)
save_plot_dir = 'predictions/prediction-lstm/' + str(datestr)
...
for currency in currencies:
    if currency == 'ETH-EUR':    
        ...
        saved_model_path = 'saved_models/lstm/' + currency       
        ...
        forecast = predict(num_prediction, model, n_seq, n_steps, n_features, look_back_focast, close_data_plot) 
