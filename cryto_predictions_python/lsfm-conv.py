...
def predict(num_prediction, model, n_seq, n_steps, n_features):
    prediction_list = close_data[-look_back:]
    
    for _ in range(num_prediction):
        x = prediction_list[-look_back:]
        x = x.reshape((-1, n_seq, 1, n_steps, n_features))
...
saved_model_path = 'lstm-trained'
save_plot_dir = 'predictions/prediction-LSTM-conv'
...
close_data = np.reshape(close_data, (close_data.shape[0], n_seq, 1, n_steps, n_features))
...
#train the model
model.fit(close_data, close_data_y, epochs=num_epochs, verbose=1)
...
forecast = predict(num_prediction, model, n_seq, n_steps, n_features)
...
plt.xlabel("Time")
plt.ylabel("Price")
plt.figtext(.9, 0.9, "Currency " + currency)
